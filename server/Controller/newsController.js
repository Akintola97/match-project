...
exports.news_trending = async (req, res) => {
  try {
    const trending_data = await axios.get(
      `https://newsdata.io/api/1/news?qInTitle=tennis&category=sports&language=en&image=1&size=9&apikey=${apiKey}`
    );
...
exports.search = async (req, res) => {
  ...
  try {
    const search_data = await axios.get(
      ...
      `https://newsdata.io/api/1/news?apikey=${apiKey}&qInTitle=${search}&category=sports&language=en&image=1&size=9`
    );

...
const Chat = () => {
  ...
  const resetUnreadCount = () => {
    if (selectedUserIds && unreadCounts[selectedUserIds]) {
      // Check if there's unread count to reset
      setUnreadCounts((prevUnreadCounts) => {
        if (!prevUnreadCounts[selectedUserIds]) {
          return prevUnreadCounts; // Return previous state if no change is needed
        }
        return {
          ...prevUnreadCounts,
          [selectedUserIds]: 0, // Reset unread count for the selected user
        };
      });
    }
  };
            ...
            <div className="pt-14 p-2 flex items-center rounded-lg w-full sticky bottom-0">
              <input
                ...
                onChange={(ev) => {
                  ...
                  resetUnreadCount(); // Reset unread count when user starts typing
                }}
